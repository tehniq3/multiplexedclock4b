...
float corectie = 1.03;  // correction factor for reading when is use multiplexing method and other causes
float t1;  
...
byte format1224 = 24;  // 0,2,4,6... for 24-hour format or 1,3,5,7.. for 12-hour format
byte hours1 = 0;
byte ceasu0 = 10; // repeat clock info
...
if (format1224%2 == 1)  // convert in 12-hour format
{
  hours1 = hours%12;
...
for (byte ceasu = 0; ceasu < ceasu0; ceasu++)
{
...
if (meniu == 9)
{
 if (!digitalRead(BTN2)) // set 12/24-hour format
 { 
 format1224++;       
 delay(200);
 }  
    ...
    for(int i = 20 ; i >0  ; i--)
    {
    if (format1224%2 == 1) displayNumber52(); // 12-hour format
    else displayNumber51();               // 12-hour format
    }         
    delay(1);

if (digitalRead(BTN1)) eliberat9 = 1;
 if (!(digitalRead(BTN1)) and (eliberat9 == 1))
  {
    meniu = 10;
    delay(500);
  }
}  // end menu for change de 12/24-hour format
